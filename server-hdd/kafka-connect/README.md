...
## Recrear solo el Sink

Para aplicar un nuevo mapping sin volver a configurar Debezium, elimine el Sink y registre solo ese conector:

```sh
curl -fsS -X DELETE http://127.0.0.1:8083/connectors/scylladb-sink
bash kafka-debizium.sh scylladb-sink
curl -s http://127.0.0.1:8083/connectors/scylladb-sink/status
```
